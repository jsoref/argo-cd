...
func modifyResourceListCmd(cmdUse, cmdDesc string, clientOpts *argocdclient.ClientOptions, allow bool, namespacedList bool) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			proj, err := projIf.Get(context.Background(), &projectpkg.ProjectQuery{Name: projName})
			errors.CheckError(err)
			var list, allowList, blockList *[]metav1.GroupKind
			var listAction, listDesc string
			var add bool
			if namespacedList {
				allowList, blockList = &proj.Spec.NamespaceResourceWhitelist, &proj.Spec.NamespaceResourceBlacklist
				listDesc = "namespaced"
			} else {
				allowList, blockList = &proj.Spec.ClusterResourceWhitelist, &proj.Spec.ClusterResourceBlacklist
				listDesc = "cluster"
			}
	...
	command.Flags().StringVarP(&listType, "list", "l", defaultList, "Use block list or allow list. This can only be 'allow' or 'block'")
	return command
}
...
func NewProjectAllowNamespaceResourceCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	desc := "Removes a namespaced API resource from the block list or add a namespaced API resource to the allow list"
...
func NewProjectDenyNamespaceResourceCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	desc := "Adds a namespaced API resource to the block list or removes a namespaced API resource from the allow list"
...
func NewProjectDenyClusterResourceCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	desc := "Removes a cluster-scoped API resource from the allow list and adds it to block list"
...
func NewProjectAllowClusterResourceCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	desc := "Adds a cluster-scoped API resource to the allow list and removes it from block list"
...
func printProject(p *v1alpha1.AppProject) {
	...
	// Print blocked namespaced resources
	rbl0 := "<none>"
	...
	fmt.Printf(printProjFmtStr, "Blocked Namespaced Resources:", rbl0)
